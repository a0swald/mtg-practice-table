...
export default function HomePage() {
  ...
  const [isUmbrelHost, setIsUmbrelHost] = useState(false);
  ...
  useEffect(() => {
    ...
    setIsUmbrelHost(window.location.hostname.toLowerCase() === 'umbrel.local');
  }, []);
  ...
  const remoteSettings = isUmbrelHost ? <>
  ...
  return <>
    <main className="safe-top min-h-[100dvh] bg-[#0d0f10] px-5 pb-8 text-white md:hidden"><div className="mx-auto w-full max-w-sm"><Header compact />
      ...
      {isUmbrelHost && <MenuLabel icon={<Settings2 size={13}/>} text="Settings" />}
      {remoteSettings}
    ...
    <main className="safe-top mx-auto hidden min-h-screen max-w-xl flex-col px-5 pb-10 md:flex"><Header />
...
function Header({ compact = false }: { compact?: boolean }) { return <div className={compact ? 'mb-8' : 'mb-9'}><p className="mb-2 text-[10px] font-black uppercase tracking-[.28em] text-cyan-300">Commander companion</p><h1 className="text-4xl font-black tracking-tight">MTG Practice Table</h1><p className="mt-2 max-w-md text-sm leading-6 text-zinc-500">Practice solo or bring the companion to a live Commander table.</p></div>; }
