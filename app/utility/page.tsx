    ...
    return (
      <SettingsShell rotation={rotation}>
        ...
        <div className="w-full max-w-md px-2">
          ...
          {backgroundMode === 'preset' && <div className="mt-6 grid grid-cols-3 gap-3 sm:grid-cols-4">{COLORS.map(color => <button key={color} aria-label={`Use ${color}`} onClick={() => onPatch(current => ({ ...current, color, backgroundImageUrl: undefined, backgroundCardName: undefined }))} style={{ backgroundColor: color }} className={`h-12 w-full rounded-xl border-2 transition-transform active:scale-95 ${player.color === color && !player.backgroundImageUrl ? 'border-white shadow-[0_0_0_2px_rgba(255,255,255,.2)]' : 'border-white/10'}`} />)}</div>}
