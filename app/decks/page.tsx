...
type CollectionResponse = { cards?: CardDefinition[]; notFound?: string[]; error?: string };
...
async function lookup(name: string): Promise<CardDefinition> {
  ...
  const json = await response.json().catch(() => ({})) as { error?: string } & Partial<CardDefinition>;
  if (!response.ok) throw new Error(json.error ?? `Card lookup failed (${response.status}).`);
  return json as CardDefinition;
}

async function lookupCollection(names: string[]): Promise<{ definitions: Map<string, CardDefinition>; notFound: string[] }> {
  ...
  for (let start = 0; start < names.length; start += 75) {
    ...
    if (!response.ok) throw new Error(json.error ?? `Scryfall batch lookup failed (${response.status}).`);

    const requestedByLower = new Map(chunk.map(cardName => [cardName.toLowerCase(), cardName]));
    for (const card of json.cards ?? []) {
      const original = requestedByLower.get(card.name.toLowerCase()) ?? card.name;
      definitions.set(original, card);
    }
    notFound.push(...(json.notFound ?? []));
  }
...
export default function DecksPage() {
  ...
  const [pendingCommander, setPendingCommander] = useState<CardDefinition>();
  const [pendingQuantities, setPendingQuantities] = useState<Map<string, number>>();
  const [resolved, setResolved] = useState<Map<string, CardDefinition>>(new Map());
  ...
  function clearPending() {
    setPendingCommander(undefined);
    setPendingQuantities(undefined);
    setResolved(new Map());
    setFailed([]);
  }

  function saveResolvedDeck(commander: CardDefinition, quantities: Map<string, number>, definitions: Map<string, CardDefinition>) {
    const cards: CardDefinition[] = [];
    quantities.forEach((quantity, cardName) => {
    ...
    const deck: SavedDeck = { id: uid(), name: name.trim(), commander, cards, createdAt: now, updatedAt: now };
    upsertDeck(deck);
    setDecks(loadDecks());
    setName('');
    setCommanderName('');
  ...
  async function importDeck() {
    ...
    try {
      ...
      if (commanderKey) {
        ...
        if (remaining > 0) quantities.set(commanderKey, remaining);
        else quantities.delete(commanderKey);
      }

      const uniqueNames = [...quantities.keys()];
      setStatus(`Resolving ${uniqueNames.length} unique cards through Scryfall…`);
      const { definitions, notFound } = await lookupCollection(uniqueNames);
      const notFoundSet = new Set(notFound.map(cardName => cardName.toLowerCase()));
      const failures: FailedCard[] = uniqueNames
        .filter(cardName => !definitions.has(cardName) || notFoundSet.has(cardName.toLowerCase()))
        .map(cardName => ({
          quantity: quantities.get(cardName) ?? 1,
          originalName: cardName,
          replacementName: cardName,
          error: `No exact Scryfall match for ${cardName}`,
        }));

      if (failures.length) {
        setPendingCommander(commander);
        setPendingQuantities(quantities);
        setResolved(definitions);
        setFailed(failures);
        setStatus(`${failures.length} card${failures.length === 1 ? '' : 's'} need manual resolution.`);
        return;
      }

      saveResolvedDeck(commander, quantities, definitions);
    } catch (error) {
  ...
  async function resolveFailedCards() {
    if (!pendingCommander || !pendingQuantities) return;
    setResolvingFailures(true);
    const nextDefinitions = new Map(resolved);
    const remaining: FailedCard[] = [];

    for (let index = 0; index < failed.length; index += 1) {
      const item = failed[index];
      const replacement = item.replacementName.trim();
      if (!replacement) { remaining.push({ ...item, error: 'Enter a card name.' }); continue; }
      setStatus(`Resolving ${index + 1}/${failed.length}: ${replacement}`);
      try {
        const definition = await lookup(replacement);
        nextDefinitions.set(item.originalName, definition);
      } catch (error) {
        remaining.push({ ...item, error: error instanceof Error ? error.message : 'Card lookup failed.' });
      }
      await new Promise(resolve => window.setTimeout(resolve, 120));
    }

    setResolved(nextDefinitions);
    setFailed(remaining);
    setResolvingFailures(false);
    if (remaining.length) {
      setStatus(`${remaining.length} card${remaining.length === 1 ? '' : 's'} still need attention.`);
      return;
    }
    try {
      saveResolvedDeck(pendingCommander, pendingQuantities, nextDefinitions);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Deck import failed.');
    }
  }
  ...
  return <main className="mx-auto min-h-screen w-[94vw] max-w-4xl py-6 sm:w-[90vw]">
    ...
    {failed.length > 0 && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"><section className="max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-amber-400/25 bg-[#17191a] p-5 shadow-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-amber-300">Card Resolver</div><h2 className="mt-1 text-2xl font-black">{failed.length} card{failed.length === 1 ? '' : 's'} need attention</h2><p className="mt-2 text-sm leading-6 text-zinc-400">Only genuine no-match cards appear here now. Temporary Scryfall/rate-limit failures stop the import instead of pretending the card does not exist.</p></div><button onClick={() => { clearPending(); setStatus('Import cancelled.'); }} className="rounded-xl bg-white/5 px-3 py-2 font-black text-zinc-400">✕</button></div><div className="mt-5 space-y-3">{failed.map((item, index) => <div key={`${item.originalName}-${index}`} className="rounded-2xl border border-white/10 bg-black/20 p-4"><div className="flex items-center justify-between gap-3"><div className="text-xs font-black uppercase tracking-wider text-zinc-500">No exact match</div><div className="rounded-lg bg-white/5 px-2 py-1 text-xs font-bold text-zinc-400">Qty {item.quantity}</div></div><div className="mt-2 text-sm text-red-200">Could not resolve <b>{item.originalName}</b></div><label className="mt-3 block text-xs font-black uppercase tracking-wider text-zinc-500">Resolve as<input autoFocus={index === 0} value={item.replacementName} onChange={event => setFailed(current => current.map((entry, i) => i === index ? { ...entry, replacementName: event.target.value } : entry))} onKeyDown={event => { if (event.key === 'Enter' && !resolvingFailures) void resolveFailedCards(); }} className="mt-1 w-full rounded-xl border border-amber-400/20 bg-black/30 px-3 py-3 text-sm normal-case text-white outline-none focus:border-amber-300" /></label>{item.error && <div className="mt-2 text-xs text-zinc-500">{item.error}</div>}</div>)}</div><div className="mt-5 flex gap-2"><button onClick={() => { clearPending(); setStatus('Import cancelled.'); }} className="flex-1 rounded-2xl border border-white/10 px-4 py-3 text-sm font-black text-zinc-300">CANCEL</button><button disabled={resolvingFailures} onClick={resolveFailedCards} className="flex-[1.5] rounded-2xl bg-amber-300 px-4 py-3 text-sm font-black text-zinc-950 disabled:opacity-40">{resolvingFailures ? 'CHECKING…' : 'RESOLVE & CONTINUE'}</button></div></section></div>}
  </main>;
}
