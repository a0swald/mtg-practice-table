...
function legalRemovalTargets(card:AICardTemplate,cards:CardInstance[]):CardInstance[]{
 const effects=parseSimpleOracleEffects(card.oracleText);
 if(effects.disablesAttackAndBlock)return cards.filter(c=>c.basePower!==undefined&&!c.combatDisabled);
 if(effects.destroysArtifactOrEnchantment)return cards.filter(c=>{const type=c.definition?.typeLine.toLowerCase()??'';return type.includes('artifact')||type.includes('enchantment');});
 if(effects.destroysNonartifactCreature)return cards.filter(c=>c.basePower!==undefined&&!(c.definition?.typeLine.toLowerCase()??'').includes('artifact'));
 if(effects.destroysNonblackCreature)return cards.filter(c=>c.basePower!==undefined&&!isBlack(c));
 if(effects.destroysCreature)return cards.filter(c=>c.basePower!==undefined);
 return [];
}
...
export function runAITurn(input:GameState,aiId:string):GameState{
 ...
 const hand=ai.aiHand??[];
 const removalCards=hand.filter(c=>c.kind==='removal'&&c.manaCost<=mana).map(card=>({card,targets:legalRemovalTargets(card,human.battlefield)})).filter(entry=>entry.targets.length>0);
 const removal=removalCards.length?removalCards[Math.floor(Math.random()*removalCards.length)]:undefined;
 const draw=chooseFromHand(hand,mana,'draw'),ramp=chooseFromHand(hand,mana,'ramp'),creature=chooseFromHand(hand,mana,'creature');
 const roll=Math.random();let card:AICardTemplate|undefined;
 if(removal&&(game.settings.difficulty==='challenging'||roll<.22))card=removal.card;else if(ramp&&roll<.42)card=ramp;else if(draw&&roll<.58)card=draw;else if(creature)card=creature;else card=removal?.card??ramp??draw;
 ...
 if(card.kind==='removal'){
   const targets=legalRemovalTargets(card,human.battlefield).sort((a,b)=>scoreCreature(b)-scoreCreature(a));
   const target=targets[0];if(!target)return beginCombat(game,aiId);
   pending={aiId,kind:'removal',cardName:card.name,manaCost:card.manaCost,typeLine:card.typeLine,oracleText:card.oracleText,targetInstanceId:target.instanceId,targetName:target.name};
