...
export function VirtualHand({
  ...
  return <section className="rounded-2xl border border-violet-400/20 bg-violet-400/[.045] p-3">
    <div className="mb-3 flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2">{canMulligan&&onMulligan&&<button onClick={onMulligan} className="rounded-lg border border-violet-300/20 bg-violet-300/10 px-2 py-1 text-[10px] font-black uppercase text-violet-100">Mulligan 7</button>}<div className={`rounded-lg px-2 py-1 text-[10px] font-black uppercase ${landPlayed?'bg-amber-400/10 text-amber-200':'bg-emerald-400/10 text-emerald-200'}`}>{landPlayed?'Land played':'Land available'}</div></div>
    </div>
...
export function VirtualResponsePicker({
  ...
  if(!open)return null;
  const responses=cards.filter(card=>{const type=card.definition?.typeLine.toLowerCase()??'';return type.includes('instant');});
  return <div className="fixed inset-0 z-[79] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm" role="dialog" aria-modal="true">
    <section className="max-h-[86vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-violet-400/30 bg-[#17181d] p-4">
      <div className="flex items-center justify-between"><div><div className="text-[10px] font-black uppercase tracking-[.18em] text-violet-300">Cast response</div><h2 className="text-xl font-black">Choose an instant from your hand</h2></div><button onClick={onClose} className="rounded-xl bg-white/10 px-3 py-2 font-black">✕</button></div>
      {responses.length===0?<div className="mt-4 rounded-xl bg-black/20 p-5 text-center text-sm text-zinc-400">No instant-speed responses are currently in your hand.</div>:<div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">{responses.map(card=><button key={card.instanceId} onClick={()=>onChoose(card)} className="text-left">{card.definition?.imageUrl&&<img src={card.definition.imageUrl} alt={card.name} className="w-full rounded-xl"/>}<div className="mt-1 truncate text-xs font-black">{card.name}</div></button>)}</div>}
    </section>
  </div>;
}
