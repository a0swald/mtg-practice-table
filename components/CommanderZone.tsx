...
export function CommanderZone({ commander, tax, damage, readOnly = false, onSelect, onCast, onReturn, onView }: { commander?: CardInstance; tax: number; damage: number; readOnly?: boolean; onSelect?: () => void; onCast?: () => void; onReturn?: () => void; onView?: (card: CardInstance) => void; }) {
  ...
  const displayCommander = commander && hydratedDefinition ? { ...commander, name: hydratedDefinition.name, definition: hydratedDefinition, basePower: hydratedDefinition.power ?? commander.basePower, baseToughness: hydratedDefinition.toughness ?? commander.baseToughness } : commander;
