...
export function GameClient() {
 ...
 return <main className="mx-auto min-h-screen w-full max-w-[90vw] px-4 pt-3 sm:px-5 lg:px-6">
  ...
  <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(300px,360px)] lg:items-start lg:gap-4 xl:grid-cols-[minmax(0,1fr)_400px]">
   <div className="min-w-0">
    ...
    <div className="my-3 flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2"><div><div className="text-xs font-black uppercase tracking-widest text-zinc-500">Turn {game.turnNumber}</div><div className="font-black">{game.activePlayerId==='player'?'Your turn':`${ai.name}'s turn`}</div></div><div className="text-right"><div className="text-xs text-zinc-500">Spells this turn</div><div className="text-xl font-black">{game.spellsCastThisTurn}</div></div></div>
    ...
    <section className="mt-3 rounded-2xl border border-white/10 bg-white/[.05] p-3 shadow-2xl"><PlayerHeader name="My Battlefield" life={human.life} onLife={d=>dispatch({type:'LIFE',playerId:'player',delta:d})} meta={`${human.graveyard.length} grave · ${human.exile.length} exile`}/><Battlefield cards={human.battlefield} onCard={cardTap} selectedIds={attackers}/>
