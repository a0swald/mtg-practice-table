...
export function SpellResolver({
  ...
  return (
    <section className="shrink-0 rounded-2xl border border-violet-400/35 bg-violet-400/[.08] p-4 shadow-lg shadow-violet-950/20">
      <div className="text-[10px] font-black uppercase tracking-[0.18em] text-violet-300">Resolve your spell</div>
      <div className="mt-1 text-xl font-black text-zinc-50">{spell.name}</div>
      <div className="mt-1 text-xs font-bold uppercase tracking-wide text-zinc-400">{spell.typeLine}</div>
      {spell.oracleText && <p className="mt-3 whitespace-pre-line rounded-xl bg-black/20 p-3 text-xs leading-5 text-zinc-300">{spell.oracleText}</p>}
      ...
      <div className="mt-4 flex items-end gap-2">
        <label className="flex-1">
          ...
          <input
            ...
            className="mt-1 w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm font-bold outline-none focus:border-violet-400"
          />
        </label>
        <button
          onClick={() => apply(() => onDamageOpponent(amount))}
          disabled={amount <= 0}
          className="min-h-11 rounded-xl bg-red-500 px-4 py-2 text-xs font-black disabled:opacity-40"
        >
          {opponentName.toUpperCase()} −{amount}
        </button>
      </div>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-zinc-400">
                        {stats ? `${stats.power}/${stats.toughness} · Damage ${card.damageMarked}` : card.definition?.typeLine ?? 'Permanent'}
                      </div>
                    </div>
                  </div>
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    <button
                      disabled={!stats || amount <= 0}
                      onClick={() => apply(() => onDamageCard(card, amount))}
                      className="rounded-lg bg-red-500/80 px-2 py-2 text-[11px] font-black disabled:opacity-30"
                    >
                      DAMAGE {amount}
                    </button>
                    <button onClick={() => apply(() => onDestroyCard(card))} className="rounded-lg bg-white/10 px-2 py-2 text-[11px] font-black">
                      DESTROY
                    </button>
                    <button onClick={() => apply(() => onExileCard(card))} className="rounded-lg bg-white/10 px-2 py-2 text-[11px] font-black">
                      EXILE
                    </button>
                  </div>
                </div>
      ...
      <p className="mt-3 text-xs leading-5 text-zinc-500">Apply only the parts of the Oracle text that actually happen. The app will not guess targets or complex rules interactions.</p>
