...
export function AIActionModal({ action, onResolve, onCounter, onCastResponse }: { action?: PendingAIAction; onResolve: (definition?: CardDefinition) => void; onCounter: () => void; onCastResponse: () => void; }) {
  const [definition, setDefinition] = useState<CardDefinition>();
  const [loading, setLoading] = useState(false);
  const [revealed, setRevealed] = useState(false);

  useEffect(() => {
    if (!action) { setDefinition(undefined); setRevealed(false); return; }
    let cancelled = false;
    setLoading(true);
    setRevealed(false);
    const revealTimer = window.setTimeout(() => { if (!cancelled) setRevealed(true); }, 900);
  ...
  } else result = `If it resolves, ${action.cardName} enters the battlefield and can provide mana according to its Oracle text.`;

  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true">
    {!revealed ? <div className="rounded-3xl border border-amber-400/25 bg-[#15191d] px-10 py-8 text-center shadow-2xl"><div className="mx-auto mb-4 h-3 w-3 animate-pulse rounded-full bg-amber-400"/><div className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-300">Smart Opponent</div><div className="mt-2 text-xl font-black text-zinc-50">Considering its next play…</div></div> :
    <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-sky-400/30 bg-[#15191d] p-4 shadow-2xl sm:p-5">
      <div className="flex gap-4">{imageUrl && <img src={imageUrl} alt={action.cardName} className="w-32 self-start rounded-xl shadow-xl sm:w-40" />}<div className="min-w-0 flex-1"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-sky-300">Smart Opponent casts</div><h2 className="mt-1 text-2xl font-black text-zinc-50">{action.cardName}</h2><div className="mt-1 text-xs font-bold uppercase tracking-wide text-zinc-400">{typeLine}</div>{loading && <div className="mt-2 text-xs text-zinc-500">Loading Scryfall card data…</div>}{oracleText && <div className="mt-3 whitespace-pre-line rounded-xl border border-white/10 bg-black/25 p-3 text-sm leading-6 text-zinc-200">{oracleText}</div>}</div></div>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-3"><button onClick={() => onResolve(definition)} className="rounded-xl bg-emerald-400 px-4 py-3 font-black text-zinc-950">RESOLVE</button><button onClick={onCastResponse} className="rounded-xl border border-violet-300/30 bg-violet-400/10 px-4 py-3 font-black text-violet-100">CAST RESPONSE</button><button onClick={onCounter} className="rounded-xl border border-sky-300/25 bg-white/10 px-4 py-3 font-black text-zinc-100">COUNTER</button></div>
    </div>}
  </div>;
}
